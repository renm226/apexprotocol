export const cloudData = {
  ...
  area: "cloud",
  eyebrow: "Infrastructure as Code Â· CI/CD Â· Platform Engineering Â· SRE",
  ...
  phases: [
    ...
    },
    {
      ...
      topics: [
        {
          name: "Terraform â€” Complete Guide",
          tag: "core",
        ...
        },
        {
          ...
          tag: "core",
          desc: "AWS-native IaC. CloudFormation templates (YAML/JSON), stack sets, nested stacks, custom resources. AWS CDK (Cloud Development Kit) â€” TypeScript, Python, Go, C# â€” generate CloudFormation from code. SAM (Serverless Application Model) for serverless.",
          master: [
    ...
    },
    {
      ...
      topics: [
        ...
        {
          name: "Kubernetes â€” Complete Guide",
          tag: "core",
  ...
  ,
      ...
      topics: [
        {
          ...
          tag: "core",
          desc: "GitHub Actions uses YAML workflow files to define automation triggered by repository events (push, pull request, schedule, workflow_dispatch). Concepts: workflows, jobs, steps, runners (GitHub-hosted or self-hosted), actions (reusable units). Workflow design: parallel jobs (jobs.<id>.needs for sequencing), job matrices for testing against multiple language/OS combinations, artifacts for passing files between jobs, caching (actions/cache) to speed up dependency installs. Secrets management: GitHub encrypted secrets, OIDC (OpenID Connect) for keyless cloud authentication â€” instead of storing AWS access keys as secrets, your workflow exchanges a short-lived OIDC token for temporary cloud credentials. Environments (production, staging) with required reviewers for deployment gates. Reusable workflows for DRY pipelines.",
          master: [
            "Write a CI pipeline that runs tests in parallel across Node 18, 20, and 22 using a job matrix",
            "Configure OIDC authentication to AWS â€” eliminate long-lived IAM access keys entirely",
            ...
            "Cache node_modules or pip cache correctly â€” understand when the cache key should invalidate",
        ...
        },
        {
          ...
          res: [
            ...
            "CNCF GitOps Working Group â€” GitOps Principles v1.0 (opengitops.dev)"
          ]
        }
      ]
    },
      ...
      topics: [
        {
          ...
          res: [
            "Prometheus documentation (prometheus.io/docs) â€” read the complete data model and PromQL sections",
            "Grafana documentation (grafana.com/docs) â€” dashboards and alerting",
            "Prometheus: Up & Running (O'Reilly â€” Brian Brazil, the Prometheus maintainer)",
            "The USE Method (Brendan Gregg â€” brendangregg.com/usemethod.html)"
          ]
        },
        {
          name: "Structured Logging and the ELK Stack",
          tag: "core",
          desc: "Logs are the event-level record of what happened in your system. Unstructured logs (plain text) are human-readable but machine-hostile. Structured logs (JSON) are machine-parseable and queryable. Log levels: TRACE, DEBUG, INFO, WARN, ERROR, FATAL. Log what happened, who requested it, how long it took, and what the outcome was. Never log credentials, PII, or secrets. The ELK/EFK stack: Elasticsearch (distributed search and analytics engine â€” stores and indexes logs), Logstash or Fluentd/Fluent Bit (log shipping and transformation â€” Fluent Bit preferred in K8s for its low memory footprint), Kibana (visualization and dashboards). Loki + Grafana (Grafana Labs' alternative): Loki stores log streams indexed only by labels (like Prometheus, not full-text) â€” far cheaper than Elasticsearch for most use cases. CloudWatch Logs (AWS), Cloud Logging (GCP), Azure Monitor Logs for cloud-native options. Log retention policies, archival to S3, cost management.",
          master: [
          ...
          res: [
            "Elasticsearch: The Definitive Guide (free online â€” elastic.co/guide/en/elasticsearch)",
            "Grafana Loki documentation (grafana.com/docs/loki)",
            "Fluent Bit documentation (docs.fluentbit.io)",
            "The Art of Monitoring (James Turnbull â€” free sections online)"
          ]
        },
        {
          ...
          res: [
            "Site Reliability Engineering (Google SRE Book â€” free at sre.google/sre-book)",
            "The SRE Workbook (Google â€” free at sre.google/workbook) â€” chapters on SLOs and alerting",
            "OpenTelemetry documentation (opentelemetry.io/docs)",
            "Distributed Systems Observability (Cindy Sridharan â€” free O'Reilly report)"
          ]
        }
      ]
    },
    {
      ...
      desc: "Cloud security is not a checklist â€” it is an architectural discipline. Misconfigurations, overpermissioned IAM roles, and unencrypted data are the leading causes of cloud breaches. This phase covers securing cloud infrastructure from first principles. Platform Engineering is the emerging discipline of building Internal Developer Platforms (IDPs) â€” the systems that abstract away infrastructure complexity so that product teams can self-service their deployments reliably.",
      topics: [
        {
          name: "AWS IAM and Cloud Security Architecture",
          tag: "advanced",
          desc: "Identity and Access Management (IAM) is the most critical security control in AWS. Every action in AWS is an API call that is authorized through IAM. Core concepts: Principal (who is acting â€” user, role, service), Action (what API call), Resource (which resource), Condition (under what context). IAM policies are JSON documents. Effect: Allow or Deny. Explicit Deny always wins. AWS managed policies (broad, convenient), customer managed policies (precise, recommended), and inline policies (tightly coupled to one identity). IAM Roles are preferable to IAM Users for all machine identities (EC2 instance profiles, Lambda execution roles, ECS task roles). Principle of least privilege: grant only the exact actions on exact resources needed. The IAM Access Analyzer identifies external resource access and unused permissions. AWS Organizations and Service Control Policies (SCPs) enforce guardrails at the organization level â€” even a root account cannot violate an SCP. AWS Control Tower for multi-account governance. Security services: GuardDuty (threat detection from CloudTrail, DNS, VPC Flow Logs), Security Hub (aggregates findings from GuardDuty, Inspector, Macie), Inspector (vulnerability scanning), Macie (S3 data classification), CloudTrail (API audit log â€” enable in every region, send to immutable S3 bucket).",
          master: [
        ...
        },
        {
          ...
          master: [
            ...
            "Scan your codebase for hardcoded secrets using TruffleHog or detect-secrets â€” fix every finding",
          ...
          res: [
            ...
            "Detect-secrets (Yelp GitHub â€” pre-commit hook for detecting secrets)"
          ]
        },
        {
          name: "Platform Engineering and Internal Developer Platforms",
          tag: "expert",
          desc: "Platform Engineering is the discipline of building Internal Developer Platforms (IDPs) â€” curated self-service systems that abstract away infrastructure complexity and give product engineers a 'golden path' to deploy, run, and observe their applications. A good IDP reduces cognitive load: developers should not need to understand Terraform, Kubernetes, and IAM to ship a feature. Backstage (Spotify, now CNCF) is the most widely adopted IDP framework: service catalog, TechDocs, scaffolder for golden-path templates, plugins for Argo workflows, PagerDuty, Snyk, and more. The Platforms model (Team Topologies): a platform team provides capabilities as a product with an SLA and a self-service API; stream-aligned teams (product engineers) are the customers. Infrastructure abstraction patterns: Crossplane (provision cloud resources as Kubernetes custom resources â€” your Kubernetes cluster becomes the single control plane for everything), Port as a developer portal, Humanitec for platform orchestration. The golden path: the approved, well-lit path for deploying a service â€” opinionated, documented, automated, and observable by default.",
          master: [
